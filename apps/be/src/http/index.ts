...
export function HttpResponse(c: AppContext) {
  return {
    ...
    badRequest: (message = "Permintaan tidak valid") =>
      c.json?.({ status: 400, message }, 400),
    unauthorized: (message = "Tidak berizin") =>
      c.json?.({ status: 401, message }, 401),
    forbidden: (message = "Akses ditolak") =>
      c.json?.({ status: 403, message }, 403),
    notFound: (message = "Tidak ditemukan") =>
      c.json?.({ status: 404, message }, 404),
    conflict: (message = "Terjadi konflik") =>
      c.json?.({ status: 409, message }, 409),
