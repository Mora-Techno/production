...
export const TodoStatusEnum = t.Union([t.Literal('pending'), t.Literal('completed')]);

export const CreateTodoDto = t.Object({
  text: t.String({ minLength: 1, description: 'Teks tugas' }),
  dueDate: t.Optional(t.String({ format: 'date-time', description: 'Tenggat waktu (ISO 8601)' })),
});
...
export const UpdateTodoDto = t.Object({
  ...
  dueDate: t.Optional(
    t.Nullable(t.String({ format: 'date-time', description: 'Tenggat waktu (ISO 8601)' })),
  ),
});
...
export const TodoQueryDto = t.Object({
  ...
  date: t.Optional(t.Literal('today', { description: 'Filter tugas dengan tenggat hari ini' })),
});
