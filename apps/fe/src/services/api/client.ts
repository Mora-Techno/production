...
export async function apiGet<T>(path: string, params?: QueryParams): Promise<TResponse<T>> {
...
export async function apiPost<T>(path: string, body?: unknown): Promise<TResponse<T>> {
...
export async function apiPut<T>(path: string, body?: unknown): Promise<TResponse<T>> {
  const res = await fetch(`${BASE_URL}${path}`, {
    method: 'PUT',
    headers: getAuthHeaders(),
    body: body !== undefined ? JSON.stringify(body) : undefined,
...
export async function apiPatch<T>(path: string, body?: unknown): Promise<TResponse<T>> {
  const res = await fetch(`${BASE_URL}${path}`, {
    method: 'PATCH',
    headers: getAuthHeaders(),
    body: body !== undefined ? JSON.stringify(body) : undefined,
